package com.example.banck_backend;
...
import com.example.banck_backend.service.BankService;
...
public class BankBackendApplication {
    ...
    CommandLineRunner commandLineRunner(BankAccountService bankAccountService){
        return args -> {
         Stream.of("amal","samia","ali").forEach(name->{
        ...
        CommandLineRunner start(CustomerRepository customerRepository,
                ...
                BankOpAccountRepository bankOpAccountRepository) {
            return args -> {

                Stream.of("omar", "Ali", "Amal").forEach(name -> {
                ...
                bankAccountRepository.findAll().forEach(acc -> {
                    for (int i = 0; i < 10; i++) {
                        ...
                        aOperation.setAmount(Math.random() * 12000);
                        aOperation.setType(Math.random() > 0.5 ? OperationType.DEBIT : OperationType.CREDIT);
